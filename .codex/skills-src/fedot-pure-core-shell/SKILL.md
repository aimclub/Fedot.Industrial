...
description: Refactor IndustrialTS modules by extracting pure planning, decision, validation, normalization, filtering, or transformation logic from OOP shells while preserving public APIs, coordinator classes, and lifecycle ownership in fedot_ind/core, benchmark/v2, examples, and mirrored tests. Use when code mixes orchestration with business rules, hides branching inside large methods, relies on stringly typed decisions, or should move toward a pure core plus thin effect shell design without breaking existing entry points.
---
...
Use this skill to keep IndustrialTS public OOP boundaries stable while moving computation-heavy rules into deterministic, testable helpers. Treat forecasters, benchmark runners, transformers, builders, repositories, and runtime adapters as shells that own lifecycle and effects; treat selection, validation, normalization, filtering, and transformation logic as pure core candidates.
...
### When a large function should become a small shell class

- Prefer a class when one function owns several related effects with shared lifecycle state, such as output paths, runtime configs, loaded datasets, generated assumptions, fitted models, and artifact writing.
- Keep the class narrow: one public orchestration method plus private steps named after the workflow stages. Move deterministic decisions into pure helpers or dataclasses instead of hiding them in the class.
- Avoid converting a function to a class only to group unrelated utilities. If there is no shared state, lifecycle boundary, or repeated collaborator wiring, extract pure functions instead.
- Preserve a compatibility wrapper around the old function name when external scripts already call it.
...
- Read [examples-from-prs.md](references/examples-from-prs.md) for repository-specific examples.
