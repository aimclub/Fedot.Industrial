...
description: Implement safe typed configuration parsing, validation, defaulting, and normalization in IndustrialTS without eval, sentinel strings such as 'None', hidden null semantics, or unchecked expected exceptions. Use when working on benchmark manifests, preset builders, model or kernel policies, checkpoint or backend selection, optional adapters, or any parser or validator that currently relies on raw dict blobs, string mode flags, or mixed parse plus effect logic.
---
...
- Identify the raw payload source: function params, JSON, YAML, repository metadata, CLI args, benchmark manifest input, or preset payload.
