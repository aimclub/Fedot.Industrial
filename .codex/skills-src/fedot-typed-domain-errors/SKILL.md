...
description: Model IndustrialTS domain states, requests, results, and failures explicitly with dataclasses, enums, narrow typed records, and structured error values instead of string flags, sentinel values, raw dict blobs, or unchecked expected exceptions. Use when refactoring fedot_ind/core, benchmark/v2, examples/rkhs_okhs, or adapter flows that currently hide state or domain failures inside None, loosely typed params, or branch-heavy procedural code.
---
...
Use this skill when an IndustrialTS change needs clearer domain language at the type level. The goal is to replace ad hoc flags, nullable state, and hidden failure modes with small explicit domain objects that make planning, validation, and runtime decisions easier to reason about and test.
