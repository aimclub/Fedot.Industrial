...
from .datasets.local_io import LocalDatasetParseError, load_local_supervised_split
from .errors import BenchmarkClassificationError
from .evaluation.markdown import dataframe_to_markdown
from .experiments.artifacts import (
    IncrementalBenchmarkArtifactWriter,
    load_incremental_run_records,
    resolve_incremental_run_id,
)
from .experiments.progress import BenchmarkProgressMonitor
...
class LocalClassificationAdapter:
    ...
    def load_dataset(self, spec: DatasetSpec) -> tuple[ClassificationDatasetRecord, ...]:
        ...
        try:
            ...
            download_if_missing = bool(options.get('download_if_missing', True))
            if not download_if_missing:
                raise BenchmarkClassificationError(str(local_error)) from local_error
            try:
