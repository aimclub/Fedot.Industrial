...
from benchmark.industrial.api import run_tsc_benchmark_from_legacy_config
...
class BenchmarkTSC(AbstractBenchmark, ABC):
    ...
    def run(self):
        if self.experiment_setup.get('use_industrial_benchmark'):
            warnings.warn(
                'BenchmarkTSC.run is delegating to benchmark.industrial. '
                'Use benchmark.industrial.run_tsc_benchmark_suite directly for new code.',
                DeprecationWarning,
                stacklevel=2,
            )
            experiment_setup = deepcopy(self.experiment_setup)
            experiment_setup.setdefault('custom_datasets', tuple(self.custom_datasets))
            return run_tsc_benchmark_from_legacy_config(experiment_setup)
