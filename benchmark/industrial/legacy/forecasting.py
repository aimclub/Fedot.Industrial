...
from benchmark.industrial.api import run_forecasting_benchmark_from_legacy_config
...
try:
    ...
    from benchmark.industrial.legacy.base import AbstractBenchmark
    from benchmark.industrial.legacy.forecasting_data import DatasetFormatting
...
class BenchmarkTSF(AbstractBenchmark, ABC):
    ...
    def _ensure_legacy_dependencies():
        if LEGACY_IMPORT_ERROR is not None:
            raise ImportError(
                ...
                'Use `use_industrial_benchmark=True` or install the legacy stack.'
    ...
    def run(self, path: str = None):
        if self.experiment_setup.get('use_industrial_benchmark'):
            warnings.warn(
                'BenchmarkTSF.run is delegating to benchmark.industrial. '
                'Use benchmark.industrial.run_forecasting_benchmark_suite directly for new code.',
                DeprecationWarning,
                stacklevel=2,
            )
