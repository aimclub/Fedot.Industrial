...
from benchmark.industrial.api import run_tser_benchmark_from_legacy_config
...
class BenchmarkTSER(AbstractBenchmark, ABC):
    ...
    def run(self):
        if self.experiment_setup.get('use_industrial_benchmark'):
            warnings.warn(
                'BenchmarkTSER.run is delegating to benchmark.industrial. '
                'Use benchmark.industrial.run_tser_benchmark_suite directly for new code.',
                DeprecationWarning,
                stacklevel=2,
            )
            experiment_setup = deepcopy(self.experiment_setup)
