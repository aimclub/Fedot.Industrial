...
def build_regime_diagnostics_frame(
    ...
    for record in result.run_records:
        key = (record.benchmark, record.dataset_name,
               record.subset, record.series_id)
        grouped_records.setdefault(key, []).append(record)

    for (benchmark, dataset_name, subset, series_id), series_runs in grouped_records.items():
        reference = next((record for record in series_runs if record.metadata.get(
        ...
        successful.sort(key=lambda record: float(
            record.metrics_summary.get(metric_name, float('inf'))))
        best_record = successful[0] if successful else None
        recommended_adapter = str(routing.get('primary_adapter', ''))
        recommended_candidates = tuple(
            str(item) for item in routing.get('candidate_adapters', ()))
        best_adapter = str(best_record.metadata.get(
            'adapter_name', '')) if best_record is not None else ''
        recommended_family = reference.metadata.get(
            'routing_recommendation_family')
        best_adapter_family = best_record.metadata.get(
            'model_adapter_family') if best_record is not None else None
        best_metric = float(
            best_record.metrics_summary[metric_name]) if best_record is not None else None
        recommendation_hit = bool(
            recommended_adapter and recommended_adapter in _routing_aliases(best_adapter))
        recommendation_available = any(
            recommended_adapter in _routing_aliases(
                str(record.metadata.get('adapter_name', '')))
...
def build_routing_family_summary_frame(
    ...
    regime_frame = build_regime_diagnostics_frame(
    ...
    frame['best_primary_metric'] = pd.to_numeric(
        frame['best_primary_metric'], errors='coerce')
    frame['routing_confidence'] = pd.to_numeric(
        frame['routing_confidence'], errors='coerce')
    ...
    grouped = (
        frame.groupby(
            ['benchmark', 'dataset_name', 'subset',
                'recommended_adapter_family', 'best_adapter_family'],
            dropna=False,
        )
        .agg(
...
def _extract_okhs_fdmd_stage_payload(metadata: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if stage_payload is None:
        return None
    required_keys = ('trajectory_transform', 'decomposition',
                     'rank_truncation', 'forecast_head')
...
def _extract_forecasting_stage_payload(metadata: dict[str, Any]) -> dict[str, Any] | None:
    stage_keys = ('trajectory_transform', 'decomposition',
                  'rank_truncation', 'forecast_head')
    stage_payload = {
...
def build_okhs_fdmd_stage_frame(result: ForecastingBenchmarkResult) -> pd.DataFrame:
    ...
    for record in result.run_records:
        ...
        prediction_diagnostics = dict(
            forecast_head.get('prediction_diagnostics') or {})
        ...
        rows.append(
            {
                ...
                'trajectory_features_shape': _stringify_stage_value(
                    trajectory.get('features_shape', trajectory.get(
                        'trajectory_matrix_shape_after'))
                ),
...
def build_hybrid_ensemble_frame(result: ForecastingBenchmarkResult) -> pd.DataFrame:
    ...
    for record in result.run_records:
        if record.status is not RunStatus.SUCCESS or not _is_hybrid_ensemble_record(record.metadata):
            continue
        branch_calibration = dict(
            record.metadata.get('branch_calibration') or {})
        ...
        rows.append(
            {
                ...
                'ensemble_weights': _stringify_stage_value(
                    record.metadata.get('ensemble_weights',
                                        ensemble_head.get('weights'))
                ),
...
def build_stage_tuning_family_frame(result: ForecastingBenchmarkResult) -> pd.DataFrame:
    ...
    frame['baseline_score'] = pd.to_numeric(
    ...
    summary = (
        frame.groupby(['benchmark', 'dataset_name',
                      'model_adapter_family', 'metric_name'], dropna=False)
        .agg(
            n_series=('series_id', 'count'),
...
def build_benchmark_leaderboard(
    ...
    if successful.empty:
        return pd.DataFrame(columns=['benchmark', 'dataset_name', 'model_name', metric_name, 'n_series', 'rank'])
    leaderboard = (
        successful.groupby(['benchmark', 'dataset_name', 'model_name'])[
            metric_name]
        .agg(['mean', 'count'])
        .reset_index()
...
def _stable_write_table(frame: pd.DataFrame, path_without_suffix: Path) -> list[ArtifactRecord]:
    ...
    tex_path.write_text(frame.to_latex(
        index=False, float_format=lambda value: f'{value:.4f}'), encoding='utf-8')
    artifacts.append(ArtifactRecord(
        kind='table', path=str(tex_path), format='tex'))

    parquet_path = path_without_suffix.with_suffix('.parquet')
    try:
        frame.to_parquet(parquet_path, index=False)
        artifacts.append(ArtifactRecord(kind='structured',
                         path=str(parquet_path), format='parquet'))
    except Exception:
        pass
    return artifacts
...
def compare_models_on_series(
    ...
    if series_predictions.empty:
        raise ValueError(
            f'No prediction records found for series_id={series_id}.')
    ...
    series_metrics = metrics[(metrics['series_id'] == series_id) & (
        metrics['horizon_index'].isna())].copy()
    series_metrics = series_metrics.sort_values(
        ['metric_name', 'metric_value', 'model_name'])
    series_predictions = series_predictions.sort_values(
        ['model_name', 'horizon_index'])
    ...
    if output_dir is not None:
        ...
        pivot = series_predictions.pivot(
            index='horizon_index', columns='model_name', values='y_pred')
        truth = (
        ...
        forecast_index = np.arange(
            len(train_values), len(train_values) + len(actual_test))
        ...
        if len(train_values):
            overlay_axis.plot(history_index, train_values,
                              label='train', linewidth=2.0, color='0.45')
        overlay_axis.plot(forecast_index, actual_test,
                          label='actual', linewidth=2.5, color='black')
        for model_name in pivot.columns:
            overlay_axis.plot(
                forecast_index, pivot[model_name].to_numpy(), label=model_name, linewidth=1.8)
        overlay_axis.axvline(len(train_values) - 1, color='red',
                             linestyle='--', linewidth=1.2, alpha=0.8)
        overlay_axis.set_title(
            f'History and Forecast Comparison for {series_id}')
        ...
        for extension in ('png', 'svg'):
            path = target_dir / \
                f'{series_id}_history_forecast_overlay.{extension}'
            overlay_figure.savefig(path, dpi=200, bbox_inches='tight')
            artifact_manifest.append(ArtifactRecord(
                kind='plot', path=str(path), format=extension))
        ...
        boundary_axis.plot(forecast_index, actual_test,
                           label='actual', linewidth=2.5, color='black')
        for model_name in pivot.columns:
            boundary_axis.plot(
                forecast_index, pivot[model_name].to_numpy(), label=model_name, linewidth=1.8)
        boundary_axis.axvline(len(train_values) - 1, color='red',
                              linestyle='--', linewidth=1.2, alpha=0.8)
        ...
        last_train_value = float(
            train_values[-1]) if len(train_values) else 0.0
        labels = ['actual'] + list(pivot.columns)
        deltas = [float(actual_test[0] - last_train_value)
                  ] if len(actual_test) else [0.0]
        deltas.extend(float(pivot[model_name].iloc[0] - last_train_value)
                      for model_name in pivot.columns)
        delta_axis.bar(labels, deltas, color=[
                       'black'] + [f'C{index % 10}' for index in range(len(pivot.columns))])
        ...
        if not horizon_metrics.empty:
            ...
            for model_name, group in horizon_metrics.groupby('model_name'):
                horizon_axis.plot(
                    group['horizon_index'], group['metric_value'], label=model_name, linewidth=1.8)
            horizon_axis.set_title(f'Horizon Error Profile for {series_id}')
            horizon_axis.set_xlabel('Horizon')
            horizon_axis.set_ylabel(
                result.aggregate_report.primary_metric.upper())
        ...
        series_run_records = [
            record for record in result.run_records if record.series_id == series_id]
        regime_reference = next(
            (record for record in series_run_records if record.metadata.get(
                'regime_diagnostics')),
            None,
        )
        if regime_reference is not None:
            ...
            diagnostics_path = target_dir / \
                f'{series_id}_regime_diagnostics.json'
            write_json(diagnostics_path, _prune_generic_artifact_payload(
                regime_payload, verbosity_policy))
            artifact_manifest.append(ArtifactRecord(
                kind='structured', path=str(diagnostics_path), format='json'))
        ...
        if stage_records:
            ...
            stage_path = target_dir / \
                f'{series_id}_forecasting_stage_diagnostics.json'
            write_json(stage_path, _prune_generic_artifact_payload(
                stage_payload, verbosity_policy))
            artifact_manifest.append(ArtifactRecord(
                kind='structured', path=str(stage_path), format='json'))
        ...
        if hybrid_records:
            hybrid_payload = {
                record.model_name: _prune_generic_artifact_payload(
                    record.metadata, verbosity_policy)
                for record in hybrid_records
            }
            hybrid_path = target_dir / \
                f'{series_id}_hybrid_ensemble_diagnostics.json'
            write_json(hybrid_path, hybrid_payload)
            artifact_manifest.append(ArtifactRecord(
                kind='structured', path=str(hybrid_path), format='json'))
        ...
        if stage_tuning_records:
            ...
            stage_tuning_path = target_dir / \
                f'{series_id}_forecasting_stage_tuning.json'
            write_json(stage_tuning_path, stage_tuning_payload)
            artifact_manifest.append(ArtifactRecord(
                kind='structured', path=str(stage_tuning_path), format='json'))
        ...
        if okhs_records:
            ...
            diagnostics_path = target_dir / \
                f'{series_id}_okhs_diagnostics.json'
            ...
            for record in okhs_records:
                stage_payload = _extract_okhs_fdmd_stage_payload(
                    record.metadata)
                if stage_payload is None:
                    continue
            ...
            if okhs_fdmd_stage_payload:
                stage_diagnostics_path = target_dir / \
                    f'{series_id}_okhs_fdmd_stage_diagnostics.json'
                write_json(
                    stage_diagnostics_path,
                    _prune_generic_artifact_payload(
                        okhs_fdmd_stage_payload, verbosity_policy),
                )
                artifact_manifest.append(
                    ArtifactRecord(kind='structured', path=str(
                        stage_diagnostics_path), format='json')
                )

            for record in okhs_records:
                fit_diagnostics = record.metadata.get(
                    'fdmd_fit_diagnostics', {})
                if not fit_diagnostics:
                    continue
                eigen_real = np.asarray(fit_diagnostics.get(
                    'eigenvalues_real', []), dtype=float)
                eigen_imag = np.asarray(fit_diagnostics.get(
                    'eigenvalues_imag', []), dtype=float)
                mode_norms = np.asarray(fit_diagnostics.get(
                    'mode_norms', []), dtype=float)
                prediction_diagnostics = record.metadata.get(
                ...
                discontinuity = prediction_diagnostics.get(
                    'boundary_discontinuity_abs_mean')
                resolved_modes = fit_diagnostics.get('resolved_n_modes')
                anti_smoothing = prediction_diagnostics.get(
                ...
                for extension in ('png', 'svg'):
                    path = target_dir / \
                        f'{series_id}_{model_slug}_okhs_modes.{extension}'
        ...
        if havok_records:
            ...
            diagnostics_path = target_dir / \
                f'{series_id}_havok_diagnostics.json'
            write_json(diagnostics_path, diagnostics_payload)
            artifact_manifest.append(ArtifactRecord(
                kind='structured', path=str(diagnostics_path), format='json'))
            ...
            for record in havok_records:
                model_slug = _slugify_model_name(record.model_name)
                forcing_values = np.asarray(record.metadata.get(
                    'forcing_values', []), dtype=float)
                forcing_threshold = float(
                    record.metadata.get('forcing_threshold', 0.0))
                forcing_mask = np.asarray(
                    record.metadata.get('forcing_mask', []), dtype=bool)
                ...
                forecast_forcing_values = np.asarray(record.metadata.get(
                    'forecast_forcing_values', []), dtype=float)
                forecast_forcing_mask = np.asarray(
                    record.metadata.get('forecast_forcing_mask', []), dtype=bool)
                ...
                if forcing_threshold > 0:
                    forcing_axis.axhline(
                        forcing_threshold, color='C3', linestyle='--', linewidth=1.0)
                    forcing_axis.axhline(-forcing_threshold,
                                         color='C3', linestyle='--', linewidth=1.0)
                for start, stop in forcing_intervals:
                    forcing_axis.axvspan(
                        start, stop + 1, color='C3', alpha=0.12)
                forcing_axis.set_title(
                    f'HAVOK Forcing Timeline: {record.model_name}')
                ...
                for extension in ('png', 'svg'):
                    path = target_dir / \
                        f'{series_id}_{model_slug}_havok_forcing_timeline.{extension}'
                ...
                if record.model_name in pivot.columns:
                    ...
                    if len(train_values):
                        event_axis.plot(history_index, train_values,
                                        label='train', linewidth=2.0, color='0.45')
                    event_axis.plot(forecast_index, actual_test,
                                    label='actual', linewidth=2.5, color='black')
                    event_axis.plot(
                        forecast_index,
                    ...
                    event_axis.axvline(
                        len(train_values) - 1, color='red', linestyle='--', linewidth=1.2, alpha=0.8)
                    event_axis.set_title(
                        f'HAVOK Event Overlay for {series_id}')
                    ...
                    for extension in ('png', 'svg'):
                        path = target_dir / \
                            f'{series_id}_{model_slug}_havok_event_overlay.{extension}'
...
def render_publication_pack(
    ...
    verbosity_policy = _resolve_result_verbosity_policy(result)
    target_dir = ensure_directory(output_dir or Path(
        result.config.artifact_spec.output_dir) / result.run_id)
    ...
    if family_filter:
        filtered_run_records = [
            record for record in result.run_records
            if getattr(record, 'family', 'unknown') in family_filter
        ]
        filtered_result = ForecastingBenchmarkResult(
            run_id=result.run_id,
            config=result.config,
            series_records=result.series_records,
            run_records=tuple(filtered_run_records),
        ...
        leaderboard = build_benchmark_leaderboard(filtered_result)
        runs_frame = runs_to_frame(filtered_result)
    else:
    ...
    if not regime_frame.empty:
        manifest.extend(_stable_write_table(
            regime_frame, aggregate_dir / 'regime_diagnostics'))
        ...
        manifest.extend(_stable_write_table(routing_evaluation,
                        aggregate_dir / 'routing_evaluation'))
        manifest.extend(_stable_write_table(routing_evaluation,
                        aggregate_dir / 'routing_family_evaluation'))
        ...
        if not routing_family_summary.empty:
            manifest.extend(_stable_write_table(
                routing_family_summary, aggregate_dir / 'routing_family_summary'))

    forecasting_stage_frame = build_forecasting_stage_frame(result)
    if not forecasting_stage_frame.empty:
        manifest.extend(_stable_write_table(
            forecasting_stage_frame, aggregate_dir / 'forecasting_stage_diagnostics'))

    hybrid_ensemble_frame = build_hybrid_ensemble_frame(result)
    if not hybrid_ensemble_frame.empty:
        manifest.extend(_stable_write_table(hybrid_ensemble_frame,
                        aggregate_dir / 'hybrid_ensemble_diagnostics'))

    stage_tuning_frame = build_stage_tuning_frame(result)
    if not stage_tuning_frame.empty:
        manifest.extend(_stable_write_table(stage_tuning_frame,
                        aggregate_dir / 'forecasting_stage_tuning'))
    ...
    if not okhs_fdmd_stage_frame.empty:
        manifest.extend(_stable_write_table(okhs_fdmd_stage_frame,
                        aggregate_dir / 'okhs_fdmd_stage_diagnostics'))
    ...
    manifest.append(ArtifactRecord(kind='structured',
                    path=str(metadata_path), format='json'))
    ...
    manifest.append(ArtifactRecord(
        kind='summary', path=str(summary_path), format='md'))

    successful_runs = runs_frame[runs_frame['status']
                                 == RunStatus.SUCCESS.value].copy()
    if not successful_runs.empty:
        ...
        successful_runs.boxplot(column=primary_metric,
                                by='model_name', ax=boxplot_axis)
        boxplot_axis.set_title(
            f'{primary_metric.upper()} Distribution by Model')
        ...
        horizon_metrics = metrics_frame[
            (metrics_frame['metric_name'] == primary_metric) & (
                metrics_frame['horizon_index'].notna())
        ].copy()
        if not horizon_metrics.empty:
            horizon_plot = (
                horizon_metrics.groupby(['model_name', 'horizon_index'])[
                    'metric_value']
                .mean()
                .reset_index()
            )
            ...
            for extension in ('png', 'svg'):
                path = aggregate_dir / \
                    f'horizon_vs_{primary_metric}.{extension}'
                horizon_figure.savefig(path, dpi=200, bbox_inches='tight')
                manifest.append(ArtifactRecord(
                    kind='plot', path=str(path), format=extension))
            plt.close(horizon_figure)
        ...
        for dataset_name in sorted(aggregate_metrics['dataset_name'].dropna().unique()):
            ...
            for metric_name in sorted(dataset_metrics['metric_name'].dropna().unique()):
                ...
                distribution_figure, distribution_axis = plt.subplots(
                    figsize=(10, 5))
                metric_frame.boxplot(column='metric_value',
                                     by='model_name', ax=distribution_axis)
                distribution_axis.set_title(
                    f'{dataset_name}: {metric_name.upper()} Distribution')
                ...
                for extension in ('png', 'svg'):
                    path = dataset_dir / \
                        f'{dataset_slug}_metric_distribution_{metric_name}.{extension}'
                ...
                ranking_axis.bar(
                    ranking_frame['model_name'], ranking_frame['metric_value'])
                ranking_axis.set_title(
                    f'{dataset_name}: Mean {metric_name.upper()} by Model')
                ...
                for extension in ('png', 'svg'):
                    path = dataset_dir / \
                        f'{dataset_slug}_model_ranking_{metric_name}.{extension}'
                ...
                if not horizon_frame.empty:
                    horizon_summary = (
                        horizon_frame.groupby(['model_name', 'horizon_index'])[
                            'metric_value']
                        .mean()
                        .reset_index()
                    )
                    dataset_horizon_figure, dataset_horizon_axis = plt.subplots(
                        figsize=(10, 5))
                    ...
                    dataset_horizon_axis.set_title(
                        f'{dataset_name}: Horizon-wise {metric_name.upper()}')
                    ...
                    for extension in ('png', 'svg'):
                        path = dataset_dir / \
                            f'{dataset_slug}_horizon_distribution_{metric_name}.{extension}'
                        dataset_horizon_figure.savefig(
                            path, dpi=200, bbox_inches='tight')
                        manifest.append(ArtifactRecord(
                            kind='plot', path=str(path), format=extension))
                    plt.close(dataset_horizon_figure)

        okhs_rows = successful_runs[successful_runs['model_name'].str.contains(
            'okhs', case=False, regex=False)]
        if not okhs_rows.empty:
            ...
            if pairwise_rows:
                pairwise_frame = pd.DataFrame(
                    pairwise_rows).sort_values('delta')
                manifest.extend(_stable_write_table(
                    pairwise_frame, aggregate_dir / 'okhs_pairwise_comparison'))

    available_series = predictions_frame['series_id'].drop_duplicates(
    ).tolist()
    for series_id in available_series[: min(3, len(available_series))]:
        comparison = compare_models_on_series(
            result, series_id=series_id, output_dir=series_dir / series_id)
