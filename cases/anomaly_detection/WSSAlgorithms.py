...
if __name__ == "__main__": 
        ...
        plot_data_scores_and_selected_window(ts_1, score_list, window_size_selected)
        print(f'{window_size_selected} - гамма-каротаж при помощи DFF')
    
        ts_12 = list(dataframe_expirement(1)['uR/h'])
        scorer_12 = WindowSizeSelection(time_series = ts_12, wss_algorithm = 'highest_autocorrelation')
        window_size_selected_12, score_list_12 = scorer_12.runner_wss()
        plot_data_scores_and_selected_window(ts_12, score_list_12, window_size_selected_12)
        print(f'{window_size_selected_12} - гамма-каротаж при помощи AC')
        
        ts_2 = list(dataframe_expirement(0)['EcgWaveform'])
        scorer_2 = WindowSizeSelection(time_series=ts_2[2500:], wss_algorithm = 'highest_autocorrelation')
        window_size_selected_2, score_list_2 = scorer_2.runner_wss()
        plot_data_scores_and_selected_window(ts_2[2500:], score_list_2, window_size_selected_2)
        print(f'{window_size_selected_2} - ЭКГ киберспортсмена за игрой в доту при помощи AC')
        
        
        ts_3 = list(dataframe_expirement(0)['EcgWaveform'])
        scorer_3 = WindowSizeSelection(time_series=ts_3[2500:])
        window_size_selected_3, score_list_3 = scorer_3.runner_wss()
        plot_data_scores_and_selected_window(ts_3[2500:], score_list_3, window_size_selected_3)
        print(f'{window_size_selected_3} - ЭКГ киберспортсмена за игрой в доту при помощи DFF')
