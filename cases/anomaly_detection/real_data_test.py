...
class DataContainer:
    def __init__(self, path: str, lables_path: str) -> None:
        ...
        if len(self.refined_data[0]) != len(self.refined_lables[0]):
            raise ValueError("Lens of labels and data have to be the same!")
        self.get_time_series_from_data()
        all_labels = ["Xu", "Yu", "Zu", "Xd", "Yd", "Zd", "Vd", "Vu"]

        # Vd Vu
        not_all_labels = ["Vd", "Vu"]
        not_all_labels = ["N"]
        self.best_score = 0
        self.best_labels = []
        self.best_window = 0
        self.best_step = 0
        self.best_filter_window = 0
        self.best_filter_thresh = 0
        attempt = 0

        self.get_several_time_series(not_all_labels)
        ...
        for L in range(0, len(all_labels) + 1):
            for subset in itertools.combinations(all_labels, L):
                if len(subset) <= 2:
        ...
        if visualize:
            print(self.processing(visualize, 100, 15, 60, 3, 10))  # 180, 24, 70, 3, 15)
            plt.show()
        ...
        try:
            ...
            if len(lables) > 1:
                ...
                if self.score > self.best_score:
                    ...
                    self.best_labels = lables
                    self.best_window = window
                    self.best_step = step
                    self.best_filter_window = filter_window
                    self.best_filter_thresh = filter_thresh
                return result
            else:
                return 0
        except:
        ...
        for file in files:
            ...
            for i in range(len(self.labels)):
                if self.labels[i][0] == filename:
                    lable = self.labels[i][1]
            temp_lable_arr = [0] * len(data)
            approx_count = 30
            for i, line in enumerate(data):
                for lable_line in lable:
                    if int(lable_line[0]) - approx_count <= line[0] + self.count_from_the_start <= int(
                            lable_line[1]) + approx_count:
                        temp_lable_arr[i] = 1
    ...
    def get_time_series_from_data(self):
        ...
        for j, data in enumerate(self.refined_data):
            self.time_series.append([])
            for _ in range(len(self.all_lables)):
