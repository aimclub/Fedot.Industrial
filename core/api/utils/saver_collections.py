...
class ResultSaver:
    def __init__(self):
        ...
        self.save_method_dict = {
                                 # 'ECM': self.save_boosting_results,
    ...
    def save_labels(self, label_data):
        path = os.path.join(default_path_to_save_results(), 'labels.csv')
        df = pd.DataFrame(label_data, dtype=int)
        df.to_csv(os.path.join(path, 'predicted_labels.csv'))

    def save_probs(self, prob_data):
        path = os.path.join(default_path_to_save_results(), 'probs.csv')
        df_preds = pd.DataFrame(prob_data.round(3), dtype=float)
        df_preds.columns = [f'Class_{x + 1}' for x in df_preds.columns]
        df_preds.to_csv(os.path.join(path))
