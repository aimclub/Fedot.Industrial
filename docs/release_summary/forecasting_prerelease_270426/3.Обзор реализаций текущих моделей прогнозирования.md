...
Этот документ описывает текущие модели прогнозирования после рефакторинга.
Фокус: какое семейство моделей, какие этапы есть внутри модели, гиперпараметры, что надо тюнить и как.

Главная идея ветки - модель должна быть понятна как граф вычислений, даже если наружу она экспортируется как удобный
shell-класс.
...
- пока реализовано просто для совместимости с старой реализацией `topo_forecaster`, в стиле `lagged_ridge_forecaster`.
