...
initial_assumptions = {
    'fourier_stat_detector': PipelineBuilder().add_node('fourier_basis').add_node('stat_detector')}
...
def read_data(path_train, path_test):
    ...
    train_data = [pd.read_csv(f'{path_train}/{i}', index_col=0).values for i in os.listdir(path_train)]
    train_data = np.concatenate(train_data, axis=1).T
    train_target = np.zeros((train_data.shape[0], 1))
    test_data = [pd.read_csv(f'{path_test}/{i}', index_col=0).values for i in os.listdir(path_test)]
...
if __name__ == "__main__":
    ...
    api_config = dict(problem='classification',
                      ...
                      industrial_strategy_params={'industrial_task': 'anomaly_detection',
