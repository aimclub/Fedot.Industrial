...
TSER_REFERENCE_TABLE = PROJECT_ROOT / "benchmark" / \
    "results" / "time_series_multi_reg_comparasion_2024.csv"
...
def build_full_run_config(name: str):
    ...
    output_dir = PROJECT_ROOT / \
        str(payload["output_dir"]) if payload.get("output_dir") else None
    ...
    if experiment == "kernel_learning_m4":
        ...
        return KernelLearningM4ExperimentConfig(
            ...
            subsets=tuple(str(item)
                          for item in payload.get("subsets", ("Monthly",))),
...
def _run_payload(name: str) -> dict[str, Any]:
    ...
    if name not in runs:
        raise ValueError(
            f"Unknown full-run name: {name}. Known: {', '.join(sorted(runs))}")
...
def _discover_tser_datasets(data_root: Path | None = None) -> tuple[str, ...]:
    ...
    datasets = tuple(
        name for name in candidates if _has_local_split(root, name))
...
def _load_tser_reference_dataset_names(path: Path = TSER_REFERENCE_TABLE) -> tuple[str, ...]:
    ...
    names = tuple(str(value)
                  for value in frame[dataset_column].dropna().unique())
