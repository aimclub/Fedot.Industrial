...
#         return torch.cat(ls, dim=dim)
#     elif isinstance(it, np.ndarray):
#         return np.concatenate(ls, axis=dim)
#     else:
...
#     print(f"Memory usage of self.dataframe is {start_memory} MB")
#
...
#     print(f"Memory usage of self.dataframe after reduction {end_memory} MB")
...
#         if isinstance(roll1, int):
#             axis1 = axis1 - np.array(roll1).reshape(1, 1)
#         else:
#             axis1 = np.array(roll1).reshape(o.shape[0], 1)
#     if roll2 is not None:
#         if isinstance(roll2, int):
#             axis2 = axis2 - np.array(roll2).reshape(1, 1)
#         else:
#             axis2 = np.array(roll2).reshape(1, o.shape[1])
#     return o[axis1, axis2]
#
#
# def roll3d(o, roll1: Union[None, list, int] = None, roll2: Union[None, list, int] = None,
#            roll3: Union[None, list, int] = None):
...
#     if roll1 is not None:
#         if isinstance(roll1, int):
#             axis1 = axis1 - np.array(roll1).reshape(1, 1, 1)
#         else:
#             axis1 = np.array(roll1).reshape(o.shape[0], 1, 1)
#     if roll2 is not None:
#         if isinstance(roll2, int):
#             axis2 = axis2 - np.array(roll2).reshape(1, 1, 1)
#         else:
#             axis2 = np.array(roll2).reshape(1, o.shape[1], 1)
#     if roll3 is not None:
#         if isinstance(roll3, int):
#             axis3 = axis3 - np.array(roll3).reshape(1, 1, 1)
#         else:
#             axis3 = np.array(roll3).reshape(1, 1, o.shape[2])
...
#     a: array-like object
#     b: may be an integer, float, str, 'rand' (for random self.data), or another array like object.
...
# def create_array(shape, fname=None, path='./self.data', on_disk=True, dtype='float32', mode='r+', fill_value='rand',
...
#         ‘c’:  Copy-on-write: assignments affect self.data in memory, but changes are not saved to disk. The file on disk is read-only.
...
#
#
# def np_save_compressed(arr, fname=None, path='./self.data', verbose=False, **kwargs):
#     assert fname is not None, 'you must provide a fname (filename)'
#     if fname.endswith('npy'):
#         fname = f'{fname}.gz'
#     elif not fname.endswith('npy.gz'):
#         fname = f'{fname}.npy.gz'
...
#
#
# def np_load_compressed(fname=None, path='./self.data', **kwargs):
#     assert fname is not None, 'you must provide a fname (filename)'
#     if fname.endswith('npy'):
#         fname = f'{fname}.gz'
#     elif not fname.endswith('npy.gz'):
...
# def np2memmap(arr, fname=None, path='./self.data', dtype='float32', mode='c', **kwargs):
...
#     if dims == -1:
#         return t[..., np.arange(t.shape[dims])[::-1].copy()]
#     elif dims == 0:
#         return t[np.arange(t.shape[dims])[::-1].copy()]
#     elif dims == 1:
#         return t[:, np.arange(t.shape[dims])[::-1].copy()]
...
#         if isinstance(o, pd.core.indexes.base.Index):
#             o = o.tolist()
#         else:
#             o = [o]
#     return o
...
#     return df._get_numeric_self.data().columns.tolist()
#
#
# def get_cat_cols(df):
#     cols = df.columns.tolist()
#     cont_cols = df._get_numeric_self.data().columns.tolist()
...
#         return o[
#             torch.arange(o.shape[0], device=o.device)[:, None, None], torch.arange(o.shape[1], device=o.device)[None, :,
#                                                                       None], idx]
#     else:
#         mask = np.isnan(o)
...
# def dummify(o: Union[np.ndarray, torch.Tensor], by_var: bool = True, inplace: bool = False, skip: Optional[list] = None,
#             random_state=None):
...
# def analyze_feature(feature, bins=100, density=False, feature_name=None, clip_outliers_plot=False,
...
#     print(
#         f"{'outliers':>{text_len}}: {((non_nan_feature < min_outliers) | (non_nan_feature > max_outliers)).mean():.1%}")
#     print(f"{'mean':>{text_len}}: {np.nanmean(feature)}")
#     print(f"{'std':>{text_len}}: {np.nanstd(feature)}")
#     print(f"{'normal dist':>{text_len}}: {normaltest(non_nan_feature, axis=0, nan_policy='propagate')[1] > .05}")
#     plt.figure(figsize=figsize)
#     if clip_outliers_plot:
#         plt.hist(np.clip(non_nan_feature, min_outliers, max_outliers), bins, density=density, color='lime',
#                  edgecolor='black')
#     else:
...
#         dtype=None,  # self.data type of the output.
...
#         dtype=int,  # self.data type of the output.
#         endpoint=False,
...
#     elif isinstance(o, pd.self.dataFrame):
...
#         return L([item[tup_id] for item in items])
#     else:
#         return items
#
#
...
#         if max_rows is not True and isinstance(max_rows, Integral):
#             pd.set_option('display.max_rows', max_rows)
#         else:
#             pd.set_option('display.max_rows', df.shape[0])
#     if max_columns:
#         old_max_columns = pd.get_option('display.max_columns')
#         if max_columns is not True and isinstance(max_columns, Integral):
...
# def ttest(self.data1, self.data2, equal_var=False):
#     "Calculates t-statistic and p-value based on 2 sample distributions"
#     t_stat, p_value = ttest_ind(self.data1, self.data2, equal_var=equal_var)
#     return t_stat, np.sign(t_stat) * p_value
#
#
# def kstest(self.data1, self.data2, alternative='two-sided', mode='auto', by_axis=None):
#     """Performs the two-sample Kolmogorov-Smirnov test for goodness of fit.
#
#     Parameters
#     self.data1, self.data2: Two arrays of sample observations assumed to be drawn from a continuous distributions. Sample sizes can be different.
#     alternative: {‘two-sided’, ‘less’, ‘greater’}, optional. Defines the null and alternative hypotheses. Default is ‘two-sided’.
#     mode: {‘auto’, ‘exact’, ‘asymp’}, optional. Defines the method used for calculating the p-value.
#     by_axis (optional, int): for arrays with more than 1 dimension, the test will be run for each variable in that axis if by_axis is not None.
#     """
#     if by_axis is None:
#         stat, p_value = ks_2samp(self.data1.flatten(), self.data2.flatten(), alternative=alternative, mode=mode)
#         return stat, np.sign(stat) * p_value
#     else:
#         assert self.data1.shape[by_axis] == self.data2.shape[by_axis], f"both arrays must have the same size along axis {by_axis}"
#         stats, p_values = [], []
#         for i in range(self.data1.shape[by_axis]):
#             d1 = np.take(self.data1, indices=i, axis=by_axis)
#             d2 = np.take(self.data2, indices=i, axis=by_axis)
...
#         return 0
#     else:
#         return np.sqrt(len(o)) * o.mean() / o.std()
#
#
...
#     if axis == 0:
#         return array[np.random.randn(*array.shape).argsort(axis=0), np.arange(array.shape[-1])[None, :]]
#     elif axis == 1 or axis == -1:
#         return array[np.arange(len(array))[:, None], np.random.randn(*array.shape).argsort(axis=1)]
