...
def NormalizeData(ts: list, return_numpy: bool = False):
    ...
    if not isinstance(ts, list):
        ...
        try:
            ...
            raise ValueError(
                F"Math utils - Normalization: An unexpected type of time series has been meet!")
...
def clean_begin_of_ts(ts: list, start_part_len: int, desirable_value: float = 0) -> list:
    ...
    if len(ts) > start_part_len:
        ...
        raise ValueError(
            f"Transformation error: Time series is too short. <{start_part_len}!")
    return ts
...
def clean_end_of_ts(ts: list, end_part_len: int, desirable_value: float = 0) -> list:
    ...
    if len(ts) > end_part_len:
        ...
        raise ValueError(
            f"Transformation error: Time series is too short. <{end_part_len}!")
    return ts
