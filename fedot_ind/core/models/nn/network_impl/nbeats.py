...
class NBeatsModel(BaseNeuralModel):
    ...
    def __init__(self, params: Optional[OperationParameters] = {}):
        ...
        self.batch_size = params.get("layers", 32)
    ...
    def _create_dataset(self, ts: np.ndarray):
        x_trajectory = HankelMatrix(time_series=ts,
                                    window_size=self.backcast_length,
                                    strides=1)
        y_trafectory = HankelMatrix(time_series=ts,
                                    window_size=self.forecast_length,
                                    strides=1)

        return x_trajectory.trajectory_matrix.swapaxes(0, 1), y_trafectory.trajectory_matrix.swapaxes(0, 1)
