...
from fedot_ind.core.architecture.settings.constanst_repository import MSE
...
class PatchTSTModel(BaseNeuralModel):
    ...
    def __init__(self, params: Optional[OperationParameters] = {}):
        ...
        self.patch_len = params.get('patch_len', 16)
        self.model_list = []

    def _init_model(self, ts):
        self.model = PatchTST(input_dim=1,
                              output_dim=None,
    ...
    def _fit_model(self, ts: InputData, split_data: bool = True):
        for sequences in ts.features:
            train_loader = self._prepare_data(sequences, split_data)
            loss_fn, optimizer = self._init_model(ts)
            self._train_loop(train_loader, loss_fn, optimizer)
    ...
    def split_data(self, input_data, validation_blocks=None):
        ...
        train_data, val_data = train_test_data_setup(train_input, validation_blocks=validation_blocks)
    ...
    def _prepare_data(self, ts, split_data: bool = True):
        if split_data:
            train_data, test_data = self.split_data(ts)
    ...
    def _predict(self, test_data):
        features = HankelMatrix(time_series=test_data.T,
                                window_size=self.patch_len).trajectory_matrix
        features = torch.from_numpy(DataConverter(data=features).convert_to_torch_format()).float()
        ...
        test_loader = torch.utils.data.DataLoader(data.TensorDataset(features, target),
                                                  batch_size=self.patch_len, shuffle=False)
        prediction = []
        for model in self.model_list:
            ...
            with torch.no_grad():
                for i, (batch_x, batch_y) in enumerate(test_loader):
                    ...
                    prediction.append(outputs.flatten().cpu().numpy())
    ...
    def predict(self,
        ...
        for sequences in test_data.features:
            y_pred.append(self._predict(sequences))
...
if __name__ == '__main__':
    train_data, test_data, dataset_name = get_ts_data('m4_daily',
                                                      forecast_length,
                                                      m4_id='D1032')
    window_length = round(train_data.features.shape[0] / 100 * 25)
    patch_len = 30
    model_dict = {'patch_tst_model':
                      PipelineBuilder().add_node('eigen_basis',
                                                 params={'window_size': window_length}).
                      add_node('patch_tst_model', params={'patch_len': patch_len,
