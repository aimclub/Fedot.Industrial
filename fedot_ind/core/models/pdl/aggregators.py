    ...
    def aggregate(
        ...
        *,
        ...
        probabilities = np.zeros(
            (similarity_matrix.shape[0], n_classes), dtype=float)
        for class_id in range(n_classes):
            mask = anchor_labels == class_id
            if np.any(mask):
                probabilities[:, class_id] = np.mean(
                    similarity_matrix[:, mask], axis=1)
...
def _ensure_supported_aggregation_config(config: PairwiseLearningConfig) -> None:
    ...
    if config.symmetric_inference:
        raise NotImplementedError(
            "symmetric_inference=True is reserved for PR3.")
