...
class DummyOperation(DataOperationImplementation):
    ...
    def transform(self, input_data: InputData) -> OutputData:
        if self.average is not None:
            transformed_features = np.average(input_data.features.reshape(-1, self.prediction_length), axis=0)
        else:
            transformed_features = input_data.features
        predict = self._convert_to_output(input_data, transformed_features, data_type=DataTypesEnum.table)
        return predict
