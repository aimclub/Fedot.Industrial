    ...
    def transform(self, diagrams_batch: torch.Tensor) -> tuple[torch.Tensor, list[str]]:
        ...
        for feature_name, feature_model in self.persistence_diagram_features_.items():
            try:
                ...
                expected_len = getattr(feature_model, 'max_homology_dim', 2) + 1
                batch_features.append(torch.zeros((B, expected_len), device=diagrams_batch.device))
...
class PersistenceDiagramFeatureExtractor(ABC):
    ...
    def _get_diagram_components(self, persistence_diagram: torch.Tensor):
        ...
        finite_d = torch.where(torch.isinf(d), torch.full_like(d, float('-inf')), d)
        max_finite_d = finite_d.max(dim=1, keepdim=True).values
        max_finite_d = torch.where(torch.isinf(max_finite_d), torch.zeros_like(max_finite_d), max_finite_d)
    ...
    def _get_gtda_grid(self, b: torch.Tensor, d: torch.Tensor, mask: torch.Tensor, dims: torch.Tensor, n_bins: int):
        ...
        for dim in range(self.max_homology_dim + 1):
            ...
            min_b = torch.where(torch.isinf(min_b), torch.zeros_like(min_b), min_b)
            ...
            steps = torch.linspace(0, 1, n_bins, device=b.device).view(1, n_bins)
...
class HolesNumberFeature(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        ...
        feature = torch.zeros(B, self.max_homology_dim + 1, device=persistence_diagram.device)
        for dim in range(self.max_homology_dim + 1):
...
class MaxHoleLifeTimeFeature(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        B, lifetimes, mask, dims = self._get_diagram_components(persistence_diagram)
        
        feature = torch.zeros(B, self.max_homology_dim + 1, device=persistence_diagram.device)
        for dim in range(self.max_homology_dim + 1):
            dim_mask = mask & (dims == dim)
            dim_lifetimes = torch.where(dim_mask, lifetimes, torch.zeros_like(lifetimes))
...
class RelevantHolesNumber(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        ...
        max_lifetimes = torch.zeros(B, self.max_homology_dim + 1, device=persistence_diagram.device)
        for dim in range(self.max_homology_dim + 1):
            dim_mask = mask & (dims == dim)
            dim_lifetimes = torch.where(dim_mask, lifetimes, torch.zeros_like(lifetimes))
...
class AverageHoleLifetimeFeature(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        ...
        for dim in range(self.max_homology_dim + 1):
            ...
            feature[:, dim] = torch.where(counts > 0, sums / counts, torch.zeros_like(sums))
...
class PersistenceEntropyFeature(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        ...
        for dim in range(self.max_homology_dim + 1):
            ...
            p = torch.where(lifetimes_sum > 0, dim_lifetimes / lifetimes_sum, torch.zeros_like(dim_lifetimes))
...
class SimultaneousAliveHolesFeature(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        ...
        for dim in range(self.max_homology_dim + 1):
            ...
            valid_overlaps = overlap_matrix & dim_mask.unsqueeze(2) & dim_mask.unsqueeze(1)
...
class AveragePersistenceLandscapeFeature(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        ...
        b = persistence_diagram[:, :, 0]
        d = persistence_diagram[:, :, 1]
        grid = self._get_gtda_grid(b, d, mask, dims, self.n_bins) # (B, D, n_bins)
        ...
        for dim in range(self.max_homology_dim + 1):
            ...
            L = torch.clamp(torch.min(dim_grid - b.unsqueeze(2), d.unsqueeze(2) - dim_grid), min=0.0)
...
class BettiNumbersSumFeature(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        ...
        for dim in range(self.max_homology_dim + 1):
            ...
            alive_dim = (b.unsqueeze(2) <= dim_grid) & (dim_grid < d.unsqueeze(2)) & dim_mask.unsqueeze(2)
...
class RadiusAtMaxBNFeature(PersistenceDiagramFeatureExtractor):
    ...
    def extract_feature_(self, persistence_diagram: torch.Tensor) -> torch.Tensor:
        ...
        for dim in range(self.max_homology_dim + 1):
            ...
            feature[:, dim] = max_idx / (self.n_bins * (self.max_homology_dim + 1))
