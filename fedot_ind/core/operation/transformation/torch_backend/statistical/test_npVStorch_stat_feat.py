...
# === Импортируем модули динамически ===
...
# === Утилита: безопасный вызов функции и измерение времени ===
...
def benchmark_all(runs=30):
    ...
    for name, t_np, t_cpu, t_gpu, t_cpu_b, t_gpu_b, match in results:
        ...
        match_str = "✅" if match is True else ("❌" if match is False else "⚠️")
        print(f"{name:30} {fmt(t_np):>10} {fmt(t_cpu):>13} {fmt(t_gpu):>13} {fmt(t_cpu_b):>18} {fmt(t_gpu_b):>18} {match_str:>10}")
