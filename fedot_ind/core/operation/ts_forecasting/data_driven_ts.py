    ...
    def forecast_recurrent(self, steps_ahead=12, singular_values=None, plot=False, return_df=False, **plotargs):
        ...
        for i in range(1, self.ts_N + steps_ahead):
            try:
                ...
                self.ts_forecast = np.append(self.ts_forecast, x[0])
        self.forecast_N = i + 1
        new_index = np.arange(steps_ahead)
        forecast_df = df(self.ts_forecast[-steps_ahead:], columns=['Forecast'], index=new_index)
        return forecast_df
...
class DataDrivenForForecastingBasisImplementation(ModelImplementation):
    ...
    #     ssa = mySSA(input_data.features)
    #     ssa.embed(embedding_dimension=self.window_size,
    #               suspected_frequency=int(self.seasonality * self.window_size / 100))
    #     ssa.decompose()
    #     plt.show()
    #     predict = ssa.forecast_recurrent(steps_ahead=forecast_length,
    #                                      singular_values=[i for i in range(min(self.n_components, ssa.r))])
    ...
    def predict(self, input_data: InputData) -> OutputData:
        ...
        s[1] = np.sqrt(s[1])
        components = self.get_combined_components(U, s, VT)
    ...
    def get_svd(self, data):
        components = Either.insert(data).then(self.decomposer.svd).value[0]
