...
def dataset():
    (X_train, y_train), (_, _) = TimeSeriesDatasetsGenerator(
        num_samples=20, max_ts_len=50, binary=False, test_size=0.5, task='regression').generate_data()
...
def test_tcn_prepare(ts, tcn):
    input_data = tcn._TCNModel__preprocess_for_fedot(ts)
    loader = tcn._prepare_data(input_data.features, patch_len=30, split_data=False)
    assert loader is not None
    assert isinstance(loader, torch.utils.data.dataloader.DataLoader)
