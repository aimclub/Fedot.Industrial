...
def test_valid_trajectory_matrix(ts_data, valid_window_size):
    ...
    assert trajectory_matrix.shape[0] == valid_window_size + 1
    assert trajectory_matrix.shape[1] == TS_LENGTH - valid_window_size
...
def test_over_half_trajectory_matrix(ts_data, over_half_window_size):
    trajectory_matrix = HankelMatrix(time_series=ts_data,
                                     window_size=over_half_window_size).trajectory_matrix
    made_up_window = int(TS_LENGTH / 3)
    assert trajectory_matrix is not None
    assert trajectory_matrix.shape[0] == made_up_window + 1
    assert trajectory_matrix.shape[1] == TS_LENGTH - made_up_window
...
@pytest.fixture
def over_half_window_size():
    return TS_LENGTH // 2 + 1
