...
DATA_TASK_DIRS = ("ts_classification", "ts_regression",
...
def test_obsolete_example_directories_are_removed() -> None:
    ...
    assert not (EXAMPLES_ROOT / "real_world_examples" /
                "benchmark_example" / "analysis of results").exists()
    assert not (EXAMPLES_ROOT / "real_world_examples" /
                "benchmark_example" / "archive").exists()
...
def test_data_task_layout_has_readmes_and_tracked_fixtures() -> None:
    ...
    assert (DATA_ROOT / "ts_regression" /
            "MadridPM10Quality-no-missing").is_dir()
...
def test_data_loader_resolves_task_based_fixture_paths() -> None:
    ...
    classification_parent = Path(resolve_dataset_parent_path(
        EXAMPLES_DATA_PATH, "ItalyPowerDemand_fake"))
    regression_parent = Path(resolve_dataset_parent_path(
        EXAMPLES_DATA_PATH, "MadridPM10Quality-no-missing"))
...
def test_automl_and_kernel_learning_config_previews_are_typed(tmp_path: Path) -> None:
    ...
    anomaly_preview = invoke_tool(
        ...
        {"folder": "valve1", "dataset": "0",
            "output_dir": str(tmp_path / "anomaly")},
    )
    ...
    assert Path(anomaly_context["data_root"]) == DATA_ROOT / \
        "anomaly_detection" / "liman" / "vibro"
    assert anomaly_preview["status"] == "dry_run"
    assert Path(anomaly_preview["data"]["target_path"]
                ).parts[-2:] == ("valve1", "0.csv")
    tool_names = {tool["name"] for tool in list_tool_specs()}
    assert {"industrial_train_model", "industrial_run_evolution",
            "industrial_detect_anomalies"}.issubset(tool_names)
    assert "industrial_tsc_smoke" in tool_names
    response = invoke_tool("industrial_forecasting_config_preview", {
                           "output_dir": str(tmp_path / "tool")})
...
def test_tools_example_exposes_mcp_ready_action_contracts(tmp_path: Path) -> None:
    ...
    train = invoke_tool(
        "industrial_train_model",
        {"task_type": "ts_classification", "dataset_name": "Lightning7",
    ...
    evolution = invoke_tool("industrial_run_evolution", {
                            "datasets": ["Lightning7"]})
    anomaly = invoke_tool("industrial_detect_anomalies", {
                          "folder": "valve1", "dataset": "0"})
...
def test_real_world_context_entrypoints_are_importable_and_do_not_load_external_data() -> None:
    ...
    kaggle = build_kaggle_forecasting_context(
        "examples/utils/data/forecasting/kaggle_inventory")
    eeg = eeg_classification_context()
    debet = build_debet_forecasting_context()

    assert skab["context"]["task_type"] == "anomaly_detection"
    assert Path(skab["context"]["data_root"]) == DATA_ROOT / \
        "anomaly_detection" / "skab"
    ...
    assert Path(debet["context"]["data_root"]
                ) == DATA_ROOT / "forecasting" / "debet"
...
def test_real_world_analysis_notebooks_are_thin_current_api_entries() -> None:
    ...
    analysis_dir = EXAMPLES_ROOT / "real_world_examples" / \
    ...
    for path in notebook_paths:
        ...
        assert not any(
            token in content for token in FORBIDDEN_NOTEBOOK_TOKENS), path

    from examples.artifacts import load_artifact_catalog

    catalog = load_artifact_catalog()
    analysis_group = next(
        group for group in catalog["groups"] if group["key"] == "benchmark_analysis_publication_packs"
    )
    analysis_showcase_keys = {item["key"]
                              for item in catalog["benchmark_showcase"]}
    assert analysis_group["storage_policy"] == "external_archive"
    assert analysis_group["local_path"].startswith(
        "examples/artifacts/cloud_bundle/")
    assert set(available_analysis_names()) <= analysis_showcase_keys


def test_real_world_notebooks_do_not_use_removed_api_tokens() -> None:
    notebook_paths = sorted(
        (EXAMPLES_ROOT / "real_world_examples").rglob("*.ipynb"))
...
def test_forecast_comparison_artifacts_are_external_archive_entries() -> None:
    from examples.artifacts import load_artifact_catalog

    catalog = load_artifact_catalog()
    forecast_entries = [
        item for item in catalog["benchmark_showcase"] if item["task_type"] == "forecasting"
    ]
    assert forecast_entries
    assert all(
        item["artifact_path"].startswith("examples/artifacts/cloud_bundle/")
...
def test_real_world_data_delivery_and_domain_scenarios_are_manifest_driven(tmp_path: Path) -> None:
    ...
    manifest = render_scenario_preview_pack(
        "eeg_classification", output_dir=tmp_path / "eeg_preview")
    ...
    assert (DATA_ROOT / "benchmark_history" /
            "real_world_archive" / "archive_manifest.json").is_file()
    ...
    assert "LaggedRidgeForecaster" in build_scenario_context("bitcoin_forecasting")[
        "models"]
