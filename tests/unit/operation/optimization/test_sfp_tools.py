...
def test_percentage_filter_zeroing():
    ...
    with pytest.raises(AssertionError):
        percentage_filter_zeroing(conv=conv, pruning_ratio=1)
    with pytest.raises(AssertionError):
        percentage_filter_zeroing(conv=conv, pruning_ratio=0)
    with torch.no_grad():
        for i, v in enumerate([0.1, 0.5, 0.7, 0.2]):
            conv.weight[i, :, :, :] = v
    percentage_filter_zeroing(conv=conv, pruning_ratio=0.5)
...
def test_energy_filter_zeroing():
    conv = torch.nn.Conv2d(3, 4, 3)
    with pytest.raises(AssertionError):
        energy_filter_zeroing(conv=conv, energy_threshold=0)
    with torch.no_grad():
        for i, v in enumerate([0.1, 0.5, 0.7, 0.2]):
            conv.weight[i, :, :, :] = v
    energy_filter_zeroing(conv=conv, energy_threshold=0.95)
    expected = torch.zeros_like(conv.weight)
    expected[1, :, :, :] = 0.5
    expected[2, :, :, :] = 0.7
    assert torch.equal(conv.weight, expected)
