...
for ind in substrings:
      ...
      end_index = next((i for i, line in enumerate(file_content[start_index:]) if line.endswith("}")), None) + 1 + start_index
      ...
      res = res + ["abstract " + ind + " {"] + [string for string in formatted_strings if string.strip() and string.strip() != "{"] + ["}", ""]
